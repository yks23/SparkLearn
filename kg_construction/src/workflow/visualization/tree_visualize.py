...
class NodeProcessor:
    ...
    def _classify_entities(self, entities: List[Dict]) -> Dict[str, List[Dict]]:
        """将实体分类"""
        classified = {"core": [], "noncore": []}

        for entity in entities:
            if entity["is_core_entity"]:
                classified["core"].append(entity)
            else:
                classified["noncore"].append(entity)
...
class RelationProcessor:
    ...
    def _classify_relations(self, relations: List[Dict]) -> Dict[str, List[Dict]]:
        ...
        #     [GraphStructureType.entity_node], return_type="object"
        # )[0]
        # entitydict = {ent.id: ent for ent in entities}

        for rel in relations:
            if (
                rel["is_tree"]
                or rel["type"] == "has_example"
                or rel["type"] == "has_chunk"
                or rel["type"] == "has_subsection"
                or rel["type"] == "has_entity"
            ):
                # if rel['target_id'] in entitydict.keys() and rel['source_id'] not in entitydict.keys():
                #         if entitydict[rel['target_id']].is_core_entity==False:
                #             continue
                classified["TREE"].append(rel)
            else:
                classified["NON_TREE"].append(rel)
        return classified
