...
def getBody(filepath):
    ...
    return data
# 语种设置
language = "cn|en"  # cn:中文 en:英文
# 是否返回文本位置信息
location = "true"
# 图片上传接口地址
picFilePath = "./source/difficult.png"
# headers=getHeader(language, location)
r = requests.post(URL, headers=getHeader(), data=getBody(picFilePath))
#print(r.content)

res_dict = json.loads(r.content.decode('utf-8'))

if res_dict["code"] == "0":
    print("识别成功！识别结果如下：\n")
    lines = res_dict["data"]["block"][0]["line"]
    for line in lines:
        line_text = ''.join([word["content"] for word in line["word"]])
        print(line_text)  # 正常显示中文
else:
    print(f"识别失败，错误码：{res_dict['code']}, 描述：{res_dict.get('desc', '无')}")